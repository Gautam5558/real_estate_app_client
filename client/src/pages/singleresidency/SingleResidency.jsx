...
const SingleResidency = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () =>
      axios
        .get(import.meta.env.VITE_BASE_URL + "api/residencies/" + residencyId)
        .then((res) => {
          return res.data;
        }),
  });
  ...
  const cancelBooking = async () => {
    try {
      const { data } = axios.put(
        import.meta.env.VITE_BASE_URL + "api/users/" + residencyId,
        {},
      ...
      <div className="wrapper">
        <div className="rTop">
          <div className="imgContainer">
            ...
            <MdOutlineNavigateNext
              ...
              onClick={() =>
                setImageIndex((prev) => {
                  return prev === data.images.length - 1 ? prev : prev + 1;
                })
              }
            />
        ...
        <div className="rBottom">
          <div className="bLeft">
            ...
            {state?.user?.bookedVisits?.some((visit) => {
              return visit.residencyId === residencyId;
            }) ? (
              <>
                <button className="rButton booked" onClick={cancelBooking}>
                  Cancel Booking
                </button>
                <span className="bookingDate">
                  You've already booked this residency for{" "}
                  {
                    state?.user?.bookedVisits?.filter((visit) => {
                      return visit.residencyId === residencyId;
                    })[0].date
                  }
                </span>
