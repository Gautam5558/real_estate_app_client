...
const Heart = ({ residencyId, card }) => {
  ...
  const handleLike = async () => {
    if (state.user === null) {
      ...
      navigate(
        "/login?message=You must be logged in to add a property to favorites"
      );
      return;
    }
  ...
  return (
    <div className="heart">
      <AiFillHeart
        className={card ? "cardLikeIcon" : "likeIcon"}
        onClick={handleLike}
        style={{
          color: state?.user?.favoriteResidencies?.some((id) => {
            return id === residencyId;
          })
            ? "red"
            : "white",
        }}
